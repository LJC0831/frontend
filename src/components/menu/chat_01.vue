<template>
  ...
  export default {
    ...
    created() {
      ...
      this.socket = io('http://158.179.164.147:3000', {
    ...
    methods: {
      ...
      async fetchThumbnail(sendUrl) {
        ...
        try {
          const response = await fetch('http://158.179.164.147:3000/api/fetchThumbnail', {
            method: 'POST',
    ...
    handleChatTextareaBlur() {
      ...
      async chatfileUrl(chat_file_id, originalFileName) {
        ...
        const messageObject = {
          ...
          profilePicture: this.profilePicture,
          chatimageUrl: `http://158.179.164.147:3000/api/file/download/${chat_file_id}`,
          chatId: this.selectedChatId,
          isMyMessage: true,
          selectUserCount: 0,
          answer_message:this.answerMessage,
