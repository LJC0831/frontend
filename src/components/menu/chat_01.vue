<template>
    ...
    <div v-if="isStickerModal" class="sticker-modal">
      ...
      <div class="sticker-selector" style="width:250px">
        <img
            ...
            :key="stickerId"
            :src="`/sticker/bear-image${stickerId}.png`"
  ...
  export default {
    ...
    created() {
      ...
      this.socket = io('https://backendserver.pe.kr:3000', {
      ...
      async fetchThumbnail(sendUrl) {
        ...
        try {
          //const response = await fetch('https://port-0-backend-nodejs-20zynm2mlk2nnlwj.sel4.cloudtype.app/api/fetchThumbnail', {
            const response = await fetch('https://backendserver.pe.kr:3000/api/fetchThumbnail', {
            method: 'POST',
    ...
    handleChatTextareaBlur() {
      ...
      async chatfileUrl(chat_file_id, originalFileName) {
        ...
        const messageObject = {
          ...
          chatimageUrl: `https://backendserver.pe.kr:3000/api/file/download/${chat_file_id}`,
          chatId: this.selectedChatId,
          isMyMessage: true,
          selectUserCount: 0,
          answer_message:this.answerMessage,
